...
// UPI details for the "Pay Now" line inside the WhatsApp message only.
// This is NOT rendered as a button anywhere on the website — it's plain
// text in the message, which WhatsApp automatically turns into a tappable
// link once the customer receives it.
...
function CheckoutPage() {
  ...
  const placeOrder = (e: React.FormEvent) => {
    ...
    const deliveryText = DELIVERY_FEE === 0 ? "To be confirmed" : formatINR(DELIVERY_FEE);

    // NOTE on "PAY NOW": WhatsApp pre-filled text messages are plain text —
    // there is no way to make the words "PAY NOW" themselves a clickable
    // link while hiding the actual URL (that HTML-style link formatting
    // isn't supported by wa.me messages). "PAY NOW" is shown as a label,
    // and the UPI link directly below it is what WhatsApp auto-detects and
    // makes tappable once the customer receives the message.
    const lines = [
      "🛒 KALA FLAVOURS ORDER",
      "",
      "Customer Details:",
      ...
      buildUpiUrl(total),
      "",
      "After payment, please send the payment screenshot here to confirm the order.",
    ];
