	...
	const plugin = {
		...
		buildApp: {
			...
			async handler() {
				...
				const payload = serializeDocumentSet(present, GENERATOR);
...
const SANDBOX_WRANGLER_CONFIG_PATH = `${SERVER_OUTPUT_DIR}/wrangler.json`;
...
function prerenderShimSource(vars) {
	return `${SHIM_MARKER}
import server from "./${NITRO_SERVER_ENTRY}";
...
function prerenderPreviewShim(state) {
	let root = process.cwd();
	let serverOutDir = SERVER_OUTPUT_DIR;
	...
	return {
		...
		config(userConfig) {
			...
			if (ssrBuild?.outDir) serverOutDir = ssrBuild.outDir;
		...
		buildApp: {
			...
			handler: async () => {
				...
				const outDir = nodePath.resolve(root, serverOutDir);
				if (!await exists(nodePath.join(outDir, NITRO_SERVER_ENTRY))) {
					logWarning(`Cannot prerender: nitro's ${NITRO_SERVER_ENTRY} is not in ${serverOutDir}, so the preview shim was not written and every prerendered route will fall back to SSR.`);
					return;
				}
				const basename = nodePath.basename(serverEntryName, nodePath.extname(serverEntryName));
				if (!basename || basename === "index") {
					logWarning(`Cannot prerender: the server entry "${serverEntryName}" would need a shim at ${SERVER_OUTPUT_DIR}/index.js, which outranks nitro's own entry when publishing. Rename the entry to anything but \`index\`.`);
					return;
				}
				const target = nodePath.join(outDir, `${basename}.js`);
				const occupant = await readFile(target, "utf8").catch(() => void 0);
				if (occupant !== void 0 && !occupant.startsWith(SHIM_MARKER)) {
					logWarning(`Cannot prerender: ${basename}.js already exists in ${serverOutDir} and was not written by this plugin.`);
					return;
				}
				let vars = {};
				try {
					const candidate = JSON.parse(await readFile(nodePath.join(outDir, NITRO_WRANGLER_CONFIG), "utf8"))?.vars;
					if (candidate && typeof candidate === "object" && !Array.isArray(candidate)) vars = candidate;
				} catch {}
				await writeFile(target, prerenderShimSource(vars));
				state.file = target;
			}
		}
	};
	...
	return async (env) => {
		...
		if (options.nitro !== false && command === "build") {
			...
			if (nitroMod) {
				...
				if (prerenderEnabled) {
					const shimState = {};
					internalPlugins.push(prerenderPreviewShim(shimState), prerenderPreviewShimCleanup(shimState));
				}
		...
		if (env.isPreview && prerenderEnabled && process.env[TSS_PRERENDERING_ENV] === "true") config = (0, vite.mergeConfig)({ preview: {
